 ...
 */
    , concat: {
        ...
        , js_foiaFiler: {
            ...
            , dest: 'build/foiaFiler/js/app.js'
            , nonull: true
          }
      }
 ...
 */
    , copy: {
        ...
        , strawberry_data: {
              expand: true
            , cwd: 'src/foiaFiler/data/'
            , src: './**'
            , dest: 'build/foiaFiler/data/'
          }
        , strawberry_img: {
              expand: true
            , cwd: 'src/foiaFiler/img/'
            , src: './**'
            , dest: 'build/foiaFiler/img/'
          }
        , strawberry_pages: {
              expand: true
            , cwd: 'src/foiaFiler'
            , src: './*.*' // HACK: files only!
            , dest: 'build/foiaFiler/'
          }
      }
    ...
    , karma: {
        ...
        , strawberry: {
            ...
            , files: [
                ...
                , 'build/common/js/common.js'
                , 'build/foiaFiler/js/app.js'

                  // tests to run
                , 'test/foiaFiler/jsunit/**/*.js'
              ]
          }
      }
 ...
 */
    , sass: {
        ...
        , strawberry: {
              files: [{
                  expand: true
                , cwd: 'src/foiaFiler/scss'
                , src: ['*.scss']
                , dest: 'build/foiaFiler/css'
                , ext: '.css'
              }]
 ...
 */
    , uglify: {
        ...
        , strawberry: {
              expand: true
            , cwd: 'build/foiaFiler/js/'
            , src: './app.js'
            , dest: 'build/foiaFiler/js/'
            , ext: '.min.js'
          }
      }
 ...
 */
  ...
  grunt.registerTask('build_foiaFiler_app', [
      'build_js_foiaFiler'
    , 'sass:foiaFiler'
    , 'sass:foiaFiler_min'
    , 'copy:foiaFiler_pages'
    , 'copy:foiaFiler_img'
    , 'copy:foiaFiler_data'
  ]);

  grunt.registerTask('build_js_foiaFiler', function() {
    // Prevent an empty app level js file from being created if the `src/`
    // directory is empty.
    if (grunt.config('concat').js_foiaFiler.src.some(function (src) {
      return glob.sync(src).length > 0;
    })) {
      grunt.task.run([
          'concat:js_foiaFiler'
